...
class EnergyDistrictEnvironment(gym.Env):
    ...
    def __init__(self, config_file="./config.yaml"):
        ...
        with open(config_file, "r") as file:
            ...
            self.thermal_storages = self.network.storage_units[self.network.storage_units.carrier == "thermal_storage"].index
            ...
            dim_observation_space = 3*96 + len(self.controllables)*2  # Zeit sin/cos und Temperatur Vorhersage für 24h + Steuerbare Komponenten * (SOC (verknüpfter) Speicher + Last im Zeitpunkt)
    ...
    def step(self, action):
        ...
        for i, controllable in enumerate(self.controllables):
            if controllable["type"] == "battery":
                ...
                cop = max(float(self.network.links_t.efficiency.loc[self.timestep, controllable["name"]]), 1e-6)
                ...
                self.network.links_t.p_set.loc[self.timestep, controllable["name"]] = p_hp_action
        ...
        # print(self.network.loads_t.p.loc[self.timestep])
        # print(self.network.links_t.p1.loc[self.timestep])
        # print(self.network.buses_t.p.loc[self.timestep])
        ...
        for actor in self.config["actors"]:
            ...
            # Baseline-Netzbezug: Lasten direkt vom Netz, WP deckt immer Last
            p_el_load = 0.0
            el_load_name = f"{actor_name}_electrical_load"
            if el_load_name in self.network.loads_t.p.columns:
                p_el_load = float(self.network.loads_t.p.loc[self.timestep, el_load_name])
            p_th_load = 0.0
            th_load_name = f"{actor_name}_thermal_load"
            if th_load_name in self.network.loads_t.p.columns:
                p_th_load = float(self.network.loads_t.p.loc[self.timestep, th_load_name])
            p_pv = 0.0
            pv_name = f"{actor_name}_pv"
            if pv_name in self.network.generators_t.p.columns:
                p_pv = float(self.network.generators_t.p.loc[self.timestep, pv_name])
            cop = 1.0
            hp_name = f"{actor_name}_heatpump"
            if hp_name in self.network.links_t.efficiency.columns:
                cop = max(float(self.network.links_t.efficiency.loc[self.timestep, hp_name]), 1e-6)

            p_grid_baseline = p_el_load + (p_th_load / cop) - p_pv

            if p_grid_baseline > 0:
                baseline_cost += p_grid_baseline * 0.28 * self.dt
            else:
                baseline_cost += p_grid_baseline * 0.08 * self.dt

            # Netzbezug durch Agenten-Steuerung und Power Flow berechnung
            power_meter = self.network.lines_t.p1.loc[self.timestep, f"{actor['name']}_electrical_demand_link"]
            if power_meter > 0:
                agent_cost += power_meter * 0.28 * self.dt
            else:
                agent_cost += power_meter * 0.08 * self.dt
    ...
    def get_obs(self, timestep):
        ...
        # Normierung auf [-1,1]
        temp = 2 * (temp_forecast - self.t_min) / (self.t_max - self.t_min) - 1
        ...
        obs = np.concatenate([sin_time, cos_time, temp] + obs_controllables).astype(np.float32)
    ...
    def set_pypsa_network(self, timerange):
        ...
        #self.network.generators_t.marginal_cost['grid power'] = self.signal_values.loc[timerange, "signal_value"]
        #self.network.generators_t.marginal_cost['grid infeed'] = 1
        ...
        for actor in self.config["actors"]:
            ...
            if f"pv_gen_{actor.get('name')}" in self.pv_generation_data.columns:
                self.network.generators_t.p_set[f"{actor.get('name')}_pv"] = self.pv_generation_data.loc[timerange, f"pv_gen_{actor.get('name')}"]
    ...
    def get_controllable_components(self):
        ...
        for actor in self.config["actors"]:
            ...
            if actor.get("P_hp_nom") and actor.get("E_th_nom"):
                ...
                p_hp_nom = self.network.links.at[hp_name, "p_nom"]
